...
/**
  * In this example we have a client that reports the uptime of a 'remote host' and a service that 'sums' the uptime across hosts
  * In the real-world we want to do this async, thus the client returns a Future[Int]
  * This is a ball-ache for testing though, so what we really want is to be able to abstract over that return type, giving us a synchronous test version of the client and service
  *
  * 1. We abstract over the type constructor, rather than returning a Future[Int] we abstract to an F[_]
  * 2. Given an Applicative exists for F, we lift the result of the call to the remote host into that context, i.e. Future(uptime(host))
  * 3. We provide a test version that uses cats.Id rather than Future.  Id is a type alias Id[A] = A, such that it provides a type-constructor for
  * any type and an instance of Id is equivalent to the 'contained' value
  */
...
/**
  * Then we can wrap our equivalently parameterized service around the client, allowing us to inject a synchronous version should we wish to
